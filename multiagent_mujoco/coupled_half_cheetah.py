...
class CoupledHalfCheetah(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, agent_conf, render_mode: str=None):
        self.metadata = {"render_modes": ["human","rgb_array","depth_array",], "render_fps": 20,}
        observation_space = gymnasium.spaces.Box(low=-numpy.inf, high=numpy.inf, shape=(35,), dtype=numpy.float32)
        mujoco_env.MujocoEnv.__init__(self, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'assets', 'coupled_half_cheetah.xml'), 5, observation_space=observation_space, render_mode=render_mode)
    ...
    def step(self, action):
        ...
        terminal = False
        return ob, reward, terminal, False, dict(reward_run1=reward_run1, reward_ctrl1=reward_ctrl1,
